                      ...
                      SizedBox(
                        ...
                        child: LocationListPanel(
                          ...
                          onBatchConfirm: (ids) async {
                            // 1. 检查不完整项
                            final allLocs =
                                ref.read(assetLocationsProvider).value ?? [];
                            final selectedLocs =
                                allLocs.where((l) => ids.contains(l.id)).toList();
                            final incompleteCount = selectedLocs.where((l) {
                              return !l.hasImage ||
                                  l.atmosphere.isEmpty ||
                                  l.colorTone.isEmpty;
                            }).length;
                            ...
                            if (incompleteCount > 0) {
                              final confirmed = await showDialog<bool>(
                                ...
                                builder: (ctx) => AlertDialog(
                                  ...
                                  content: Text(
                                    '选中的 ${ids.length} 个场景中，有 $incompleteCount 个信息不完整（缺少图片、氛围或色调）。\n\n确认后仍可补充，是否继续？',
                            ...
                            try {
                              ...
                              // rethrow; // void callback, no need to rethrow
                            }
                          },
                        ),
                      ),
