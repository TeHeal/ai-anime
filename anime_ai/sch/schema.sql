...
-- 通知表（Notification，2.6 站内通知中心、任务完成写入、红点/角标）
...
-- 审核记录表（ReviewRecord，2.2 审核闭环、状态机、反馈给生产 AI）
...
CREATE INDEX idx_review_records_project_id ON review_records (project_id);

-- 定时任务表（Schedule，2.1 定时任务、按计划触发流水线）
...
CREATE INDEX idx_composite_tasks_project_id ON composite_tasks (project_id);
CREATE INDEX idx_composite_tasks_episode_id ON composite_tasks (episode_id);
CREATE INDEX idx_composite_tasks_deleted_at ON composite_tasks (deleted_at);

-- 按集打包任务表（README 2.7 生成物下载，可配置）
...
CREATE INDEX idx_package_tasks_project_id ON package_tasks (project_id);
CREATE INDEX idx_package_tasks_episode_id ON package_tasks (episode_id);
CREATE INDEX idx_package_tasks_deleted_at ON package_tasks (deleted_at);

-- 资产版本表（AssetVersion，2.4 资产阶段版本、可沉淀可复用）
...
CREATE INDEX idx_asset_versions_project_id ON asset_versions (project_id);
CREATE INDEX idx_asset_versions_deleted_at ON asset_versions (deleted_at);

-- AI 用量统计表（ProviderUsage，8.3 AI 成本控制）
...
CREATE INDEX idx_provider_usages_project_id ON provider_usages (project_id);
...
CREATE INDEX idx_resources_modality ON resources (modality);
CREATE INDEX idx_resources_library_type ON resources (library_type);
