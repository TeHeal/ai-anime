...
final assetOverviewProvider = Provider<AssetOverviewData>((ref) {
  ...
  final charNoImage = chars
      .where((c) => !c.hasImage && c.referenceImages.isEmpty)
      .length;
  final charNoVoice = chars
      .where((c) => c.voiceName.isEmpty && c.roleType != 'narrator')
      .length;
  ...
  final voiceNeeded = chars.where((c) => c.roleType != 'narrator').length;
  final voiceConfigured = chars
      .where((c) => c.voiceName.isNotEmpty && c.roleType != 'narrator')
      .length;

  final totalAssets = chars.length + locs.length + props.length;
  final totalConfirmed = charConfirmed + locConfirmed + propConfirmed;
  final readinessPct = totalAssets > 0
      ? (totalConfirmed * 100 / totalAssets).round()
      : 0;
