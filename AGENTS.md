...
术语表详见 [README §术语表](README.md#术语表优先阅读)。

**核心流程**：剧本 → 资产 → 脚本(审核) → 镜图(审核) → 镜头(审核) → 成片 → 导出

**跨模块规则**（详见 [README §跨模块调用规则](README.md#跨模块调用规则ai-须遵循)）：相同层级模块可相互依赖（引用 Service）；禁止直接引用 Data；接口解耦可选；跨领域时经 pub 编排。
...
## 4. 服务与启动（基础见 [README §启动命令](README.md#开发与部署启动命令)）
