...
| 剧本  | 集-场-块结构的故事文本           |
| 资产  | 角色、场景、道具、风格等资源         |
| 场 (Scene) | 剧本中的「场」，属于集的子结构；`scene.Location` 为文本描述（如「咖啡厅」），非资产引用 |
| 场景资产 (Location) | 场景资产实体，可被剧本/脚本引用，描述室内外、氛围、色调等 |
...
| **设计原则** | Agent驱动; 流水线思维；资产优先；模型可配置；流程顺畅；效率优先；渐进加载                              |
...
| #   | 原则             | 含义                          |
| --- | -------------- | --------------------------- |
| 1   | **AI 驱动**      | 生产各环节由 AI 主导，人工"协助"审核与异常决策  |
| 2   | **任务编排**       | 支持任务编排与定时任务                 |
...
**资产模块分类**（后端 `module/` 对应关系）：

| 分类       | 模块           | 作用域   | 说明                     |
| ---------- | -------------- | -------- | ------------------------ |
| 项目级资产 | character、location、prop、style | ProjectID | 角色、场景、道具、风格   |
| 用户级素材 | resource       | UserID   | 素材库，跨项目复用       |
| 版本管理   | asset_version  | ProjectID | Freeze/Unfreeze、版本快照 |
...
| **模块间禁止直接引用 Data** | `module/shot_image/` 的 Service **不得**直接调用 `module/assets/character/` 的 Data；跨模块访问应通过对方模块的 Service 或接口                       |
...
**示例**：镜图生成需角色信息时，`module/shot_image/service.go` 可调用 `module/assets/character/service.go` 的公开方法，但不得直接 import `module/assets/character/data.go`；若需解耦，可依赖注入 `CharacterReader` 接口，由 `pub/` 编排层注入实现。
...
│   │   └── config/           # 配置
│   └── pub/                  # 公共层
│       ├── const/            # 常量（含 routes.dart 路径）
│       ├── theme/            # 主题、颜色、设计令牌（详见 docs/设计系统.md）
│       ├── utils/            # 工具函数
...
| **pub 与 module 差异** | module 子目录=业务资源+层（`character/providers/`）；pub 子目录=能力类型（`widgets/`、`providers/`、`const/` 等），其下跨模块文件用 `资源_层` 便于检索；路由集中于 `lib/route.dart` |
