...
func (d *DBData) CreateMember(m *ProjectMember) error {
	...
	jobRolesJSON := []byte("[]")
	if len(m.JobRoles) > 0 {
		jobRolesJSON, _ = json.Marshal(m.JobRoles)
	}
	arg := db.CreateProjectMemberParams{
		ProjectID: projUUID,
		UserID:    userUUID,
		Role:      m.Role,
		JobRoles:  jobRolesJSON,
	}
...
func (d *DBData) UpdateMemberJobRoles(projectID, userID string, jobRoles []string) error {
	ctx := context.Background()
	projUUID := pkg.StrToUUID(projectID)
	userUUID := pkg.StrToUUID(userID)
	if !projUUID.Valid || !userUUID.Valid {
	...
	jobRolesJSON, _ := json.Marshal(jobRoles)
	_, err := d.q.UpdateProjectMemberJobRoles(ctx, db.UpdateProjectMemberJobRolesParams{
		JobRoles: jobRolesJSON, ProjectID: projUUID, UserID: userUUID,
	})
	return err
}
...
func dbMemberToMember(row *db.ProjectMember) *ProjectMember {
	m := &ProjectMember{
		...
		UserIDStr:    pkg.UUIDToStr(row.UserID),
		Role:         row.Role,
		JobRoles:     parseJobRoles(row.JobRoles),
	}
	if row.CreatedAt.Valid {
		m.CreatedAt = row.CreatedAt.Time
	}
...
func parseJobRoles(b []byte) []string {
	if len(b) == 0 {
		return nil
	}
	var arr []string
