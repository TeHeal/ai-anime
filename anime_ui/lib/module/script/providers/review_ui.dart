...
class ReviewUiNotifier extends Notifier<ReviewUiState> {
  ...
  void navigateShot(int delta) {
    ...
    final idx = shots.indexWhere(
      (s) => s.shotNumber == state.selectedShotNumber,
    );
...
final reviewUiProvider = NotifierProvider<ReviewUiNotifier, ReviewUiState>(
  ReviewUiNotifier.new,
);
...
ShotV4? reviewCurrentShot(ReviewUiState ui, Map<String, List<ShotV4>> shotsMap) {
  final shots = reviewCurrentShots(ui, shotsMap);
  if (ui.selectedShotNumber == null && shots.isNotEmpty) return shots.first;
  return shots.where((s) => s.shotNumber == ui.selectedShotNumber).firstOrNull;
}
